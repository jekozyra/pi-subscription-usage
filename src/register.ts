...
import { resolveClaudeSubscriptionAuthentication } from "./claude-code-auth.ts";
import {
...
export function registerWeeklySubscriptionUsage(
    ...
    await run(
      session.start({
        ...
        present: (statuses, currentTime) =>
          Effect.sync(() => {
            const lines = presentSubscriptionUsageLines(statuses, currentTime);
            const rendered = [lines.claude, lines.codex]
              .map((segments) =>
                segments
                  .map(({ text, color }) => ctx.ui.theme.fg(color, text))
                  .join(""),
              )
              .join("\n");
  ...
  pi.on("agent_settled", async (_event, ctx) => {
    ...
    if (ctx.model?.provider === "openai-codex") {
      ...
      ctx.model?.provider === "pi-claude-code-provider"
    ) {
