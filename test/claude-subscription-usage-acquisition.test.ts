...
it.effect("decodes the five-hour session window", () =>
  Effect.gen(function* () {
    const usage = yield* acquire(
      async () =>
        new Response(
          JSON.stringify({
            ...
            five_hour: {
              utilization: 42.4,
              resets_at: "2026-09-13T19:30:00Z",
            },
          }),
        ),
    );
    assert.equal(usage.sessionUsedPercent, 42.4);
    assert.equal(usage.sessionResetsAtMs, Date.parse("2026-09-13T19:30:00Z"));
  }),
);
