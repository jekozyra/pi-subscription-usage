...
test("provider names and independently colored details compose without a separator", async () => {
  ...
  assert.equal(
    ...
    "[dim:Claude  ][dim:session ][success:42% · resets 8m][dim:      week ][success:80% · resets 16m]\n[dim:Codex   ][dim:week ][success:63% · resets 16m][accent:      limit resets available: 2]",
  );
  await f.emit("session_shutdown");
});
...
test("Claude Code provider activity refreshes Claude usage", async () => {
  const f = registerFixture();
  f.setClaudeAcquisition(Effect.fail(new ClaudeAuthenticationUnavailable()));
  await f.emit("session_start");
  assert.deepEqual([f.observedCredentials.length, f.claudeReads()], [1, 1]);
  ...
  f.setProvider("pi-claude-code-provider");
  await f.emit("agent_settled");

  assert.deepEqual([f.observedCredentials.length, f.claudeReads()], [1, 2]);
  await f.emit("session_shutdown");
});
...
test("responses from another provider are ignored", async () => {
  ...
  assert.equal(
    f.statuses.at(-1)?.text,
    "Claude  session 42% · resets 8m      week 80% · resets 16m\nCodex   week 63% · resets 16m      limit resets available: 2",
  );
  await f.emit("session_shutdown");
});
